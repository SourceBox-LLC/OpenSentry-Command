...
from ..models.database import RateLimit
from ..auth import authenticate, get_client_ip, is_using_defaults, log_logout
...
def login():
    ...
    ip = get_client_ip()
    limited, remaining = RateLimit.is_locked(ip)
    if limited:
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if is_using_defaults():
                flash(
                    "⚠️ Security Warning: You are using default credentials. Please change them in your .env file!",
                    "error",
                )
            ...
            if limited:
                flash(
                    f"Account locked for {remaining} seconds due to too many failed attempts.",
                    "error",
                )
            else:
...
def settings():
    ...
    return render_template(
        "settings.html", opensentry_secret=secret, username=Config.OPENSENTRY_USERNAME
    )
...
def regenerate_secret():
    ...
    if not os.path.exists(env_path):
        audit_log(
            "SECRET_REGENERATE_FAILED", ip, current_user.username, "No .env file found"
        )
